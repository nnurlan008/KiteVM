 ...
 * [Hermit] utils
 */
bool hermit_try_to_unmap_one(struct vpage *vpage, struct page *page,
			     enum ttu_flags flags)
{
	...
	// BUG_ON(address == -EFAULT);
	if (address == -EFAULT || address != vpage->address)
		return false;
	/*
	 * For THP, we have to assume the worse case ie pmd for invalidation.
	 * For hugetlb, it could be much worse if we need to do pud
	...
	if (!hermit_addr_vma_walk(&pvmw, true)) {
		pr_err("hermit_addr_vma_walk failed!\n");
		goto walk_done;
	}
	if (pvmw.pte != vpage->pte) {
		pr_err("pte pointer doesn't match %p != %p", vpage->pte, pvmw.pte);
	}
	pte = pvmw.pte;
	...
	// [Hermit] doesn't support huge page, so subpage here
	// is always the same as page.
	subpage = page;
	...
	if (PageHuge(page) && !PageAnon(page)) {
		 ...
		 */
		VM_BUG_ON(!(flags & TTU_RMAP_LOCKED));
		if (huge_pmd_unshare(mm, vma, &address, pte)) {
			/*
			 * huge_pmd_unshare unmapped an entire PMD
			 * page.  There is no way of knowing exactly
			 * which PMDs may be cached for this mm, so
			 ...
			 enum ttu_flags flags)
{
	...
	BUG_ON(flags & TTU_RMAP_LOCKED);
	BUG_ON(flags & TTU_SPLIT_HUGE_PMD);

	if (page_not_mapped(page))
		return;
	...
	if (!hermit_try_to_unmap_one(vpage, page, flags)) {
		struct rmap_walk_control rwc = {
			.rmap_one = try_to_unmap_one,
			.arg = (void *)flags,
			.done = page_not_mapped,
		...
		// NOTE: serve as fallback but should only happen very rarely
